...
public class QueryRequest {
	public static String RegisterUser(UserModule ur) {
		...
		try {
			...
			PreparedStatement myStmt = con
					.prepareStatement("Select username, ID from Users where username= ? or " + "ID =?");
		...
		try {
			Connection con = ConnectionProvider.getCon();
			PreparedStatement myStmt = con
					.prepareStatement("Insert into Users (username,password,ID)" + "Values (?,?,?)");
			myStmt.setString(1, ur.getUsername());
	...
	public static String AddPatient(Patient p) {
		...
		try {
			...
			PreparedStatement myStmt = con
					.prepareStatement("Insert into Patients(firstname,lastname,dob,addr,phonenumber,conditions)"
							+ " Values(?,?,?,?,?,?)");
	...
	public static String DeleteEmployee(Employee e) {
		String verify = "";
		PreparedStatement myStmt;
		int count;
		try {
			Connection con = ConnectionProvider.getCon();
			System.out.println("Connected to Microsft SQL SERVER:Deleting Employee..");

			if (e.getJobTitle().equals("doctor")) {
				myStmt = con.prepareStatement("delete from Practitioners where practitionerID = ?");
				...
				myStmt = con.prepareStatement("delete from Nurses where nurseID = ?");
				myStmt.setInt(1, e.getID());
				count = myStmt.executeUpdate();
				if (count > 0) {
					verify = "success";
		...
		try {
			...
			if (e.getJobTitle().toString().equals("doctor")) {
				myStmt = con.prepareStatement("Update Practitioners Set firstname=?"
						+ ", lastname=?, dob=?, addr=?,phonenumber=?, specialty =? " + "where practitionerID=?");
				myStmt.setString(1, e.getFirstName());
				myStmt.setString(2, e.getLastName());
				myStmt.setString(3, e.getDOB());
				myStmt.setString(4, e.getAddress());
				...
				myStmt = con.prepareStatement(
						"Update Nurses Set firstname=?" + ",lastname=?, dob=?, addr=?,phonenumber=? where nurseID=?");
				myStmt.setString(1, e.getFirstName());
				myStmt.setString(2, e.getLastName());
				myStmt.setString(3, e.getDOB());
				myStmt.setString(4, e.getAddress());
				...
				myStmt = con.prepareStatement(
						"Update Admins Set firstname=?" + ", lastname=?, dob=?, addr=?,phonenumber=? where adminID=?");
				myStmt.setString(1, e.getFirstName());
				myStmt.setString(2, e.getLastName());
				myStmt.setString(3, e.getDOB());
				myStmt.setString(4, e.getAddress());
	...
	public static String AddRoom(Room r) {
		...
		try {
			...
			PreparedStatement myStmt = con
					.prepareStatement("Insert into Rooms(roomID,bedcount,type,status)" + " Values(?,?,?,?)");
			myStmt.setInt(1, r.getRoomID());
	...
	public static String AddBed(Bed b) {
		...
		try {
			...
			PreparedStatement myStmt = con
					.prepareStatement("Insert into Beds(bedID, roomID, status)" + " Values(?,?,?)");
		...
		try {
			...
			while (res.next()) {
				Patient p = new Patient(res.getInt("patientID"), res.getString("firstname"), res.getString("lastname"),
						LocalDate.parse(res.getString("dob")), res.getString("addr"), res.getString("phonenumber"),
	...
	public static ObservableList<Employee> GetAllDoctors() {
		...
		try {
			...
			ResultSet res = myStmt.executeQuery();
			while (res.next()) {
				Employee e = new Employee(res.getInt("practitionerID"), res.getString("firstname"),
						res.getString("lastname"), LocalDate.parse(res.getString("dob")), res.getString("addr"),
	...
	public static ObservableList<Employee> GetAllEmployees() {
		...
		try {
			...
			PreparedStatement myStmt = con.prepareStatement(
					"select adminID as 'ID', firstname, lastname, dob, addr, phonenumber, 'admin' as 'jobtitle'  from Admins\n"
							+ "union \n"
	...
	public static ObservableList<Room> GetAllRooms() {
		...
		try {
			Connection con = ConnectionProvider.getCon();
			System.out.println("Connected to Microsft SQL SERVER:Getting all employees..");
			PreparedStatement myStmt = con.prepareStatement("select *from Rooms");

			ResultSet res = myStmt.executeQuery();
			while (res.next()) {
				Room e = new Room(res.getInt("roomID"), res.getInt("bedcount"), res.getString("type"),
	...
	public static ObservableList<Appointment> GetApproved() {
		ObservableList<Appointment> approved = FXCollections.observableArrayList();
		try {
			Connection con = ConnectionProvider.getCon();
			System.out.println("Connected to Microsft SQL SERVER:Getting all employees..");
			PreparedStatement myStmt = con.prepareStatement("select *from Appointments where status = 'approved'");

			ResultSet res = myStmt.executeQuery();
			while (res.next()) {
				Appointment e = new Appointment(res.getInt("appID"), res.getInt("patientID"),
						res.getInt("practitionerID"), res.getInt("roomID"), LocalDate.parse(res.getString("appDate")),
						LocalTime.parse(res.getString("appTime")), res.getString("status"));

				approved.add(e);
			}
		} catch (SQLException ex) {
			System.out.println("Error: Getting approved appointments..");
			ex.printStackTrace();
		}
		return approved;
	}

	public static ObservableList<Appointment> GetWaitlisted() {
		ObservableList<Appointment> waitlisted = FXCollections.observableArrayList();
		try {
			Connection con = ConnectionProvider.getCon();
			System.out.println("Connected to Microsft SQL SERVER:Getting all employees..");
			PreparedStatement myStmt = con.prepareStatement("select *from Appointments where status = 'waitlisted'");
			...
			System.out.println("Error: Getting waitlisted appointments..");
			ex.printStackTrace();
		}
		return waitlisted;
	}
	...
	public static String RequestAppointment(Appointment a) {
		...
		try {
			...
			PreparedStatement myStmt = con.prepareStatement("Insert into Appointments"
					+ "(patientID,practitionerID,roomID,appDate,appTime,status)" + " Values(?,?,?,?,?,?)");
			myStmt.setInt(1, a.getPatientID());
			myStmt.setInt(2, a.getPractitionerID());
			myStmt.setInt(3, a.getRoomID());
			myStmt.setString(4, a.getAppDate());
			myStmt.setString(5, a.getAppTime());
